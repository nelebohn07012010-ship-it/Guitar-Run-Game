...
const Player = () => {
  ...
  useEffect(() => {
    ...
    const animate = (time: number) => {
      if (lastTime.current !== 0) {
        ...
        const gravity = 1500
        const newVerticalSpeed = verticalSpeedRef.current - gravity * deltaSeconds
        const newYPosition = yPositionRef.current + newVerticalSpeed * deltaSeconds
  ...
  return (<div id="player" style={{ bottom: yPosition }}></div>)
}
